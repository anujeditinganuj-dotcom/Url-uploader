...
async def admin_handle_cookie_file(client, message):
    ...
    # Only process if this admin is in waiting mode
    ...
    try:
        ...
        # Relaxed validation: accept if has tabs (domain\tFALSE\t... format)
        # OR has Netscape/HTTP Cookie header, OR has typical cookie domain lines
        has_header = (
            "HTTP Cookie File" in cookies_content
            or "Netscape" in cookies_content
            or "netscape" in cookies_content.lower()
        )
        has_tabs = "\t" in cookies_content
        # Also accept if lines look like cookie entries: domain flag path secure expiry name value
        lines_sample = [l.strip() for l in cookies_content.splitlines() if l.strip() and not l.startswith("#")]
        has_cookie_lines = any("\t" in l for l in lines_sample[:10])
        ...
        if not has_header and not has_tabs and not has_cookie_lines:
            await status_msg.edit(
                ...
                "2. Login to the site\n"
                "3. Cookie-Editor → **Export → Export as Netscape**\n\n"
                "Use /setcookie or Cookie Panel to try again."
            )
            try:
                os.remove(file_path)
            except Exception:
        ...
        if success:
            total_lines = len(cookies_content.strip().splitlines())
            cookie_lines = len([l for l in cookies_content.splitlines() if l.strip() and not l.startswith("#")])
            await status_msg.edit(
                f"✅ **Cookies saved successfully!**\n"
                f"📄 Total lines: **{total_lines}** (Cookie entries: {cookie_lines})\n\n"
